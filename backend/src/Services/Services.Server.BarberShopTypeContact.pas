unit Services.Server.BarberShopTypeContact;

interface

uses
  Services.Base, FireDAC.Comp.Client, System.SysUtils, Parameters.Interf, Parameters.Default, Helpers.HorseList, JSON,
  Models.Server.BarberShopTypeContact, System.Generics.Collections, SQL.Server.BarberShopTypeContact, Firedac.Stan.Async,
  REST.JSON, Validators.Server.BarberShopTypeContact, Models.Message, Enum.Message.Types, Horse.Commons;
...
begin
  ...
  try
    try
      ...
      FRes.Status(THTTPStatus.NoContent);
    except
      on E: Exception do
      begin
        ...
        FMessage.Title := 'Erro';
        FRes.Status(THTTPStatus.InternalServerError).Send<TJSONObject>(TJson.ObjectToJsonObject(FMessage, [joIgnoreEmptyStrings, joIgnoreEmptyArrays]))
      end;
    end;
